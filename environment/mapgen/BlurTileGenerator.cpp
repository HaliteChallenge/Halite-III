...
long
BlurTileGenerator::blur_function(hlt::dimension_type y_coord, hlt::dimension_type x_coord, const hlt::Map &map) const {
...
hlt::Map BlurTileGenerator::generate(std::list<hlt::Player> &players) {
    ...
    for (auto row = 0; row < tile_height; ++row) {
        for (auto col = 0; col < tile_width; ++col) {
            ...
            auto production = (unsigned long) (rng() / (double) std::mt19937::max() * (MAX_CELL_PROD - MIN_CELL_PROD) +
                                               MIN_CELL_PROD);
    ...
    const auto factory_pos_x = (hlt::dimension_type) (rng() / (float) std::mt19937::max() * tile_width);
    const auto factory_pos_y = (hlt::dimension_type) (rng() / (float) std::mt19937::max() * tile_height);
