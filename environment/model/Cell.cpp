...
void from_json(const nlohmann::json &json, Cell &cell) {
    ...
    if (type == "normal") {
        cell = std::make_shared<NormalCell>(json);
    } else if (type == "obstacle") {
        cell = std::make_shared<ObstacleCell>(json);
    } else if (type == "energy_factor") {
        cell = std::make_shared<EnergyFactorCell>(json);
    } else if (type == "factory") {
        cell = std::make_shared<FactoryCell>(json);
    } else {
        // TODO: error case
    }
}
...
ProductionCell::ProductionCell(const nlohmann::json &json) : _production(json.at("production").get<long>()) {}
...
EnergyFactorCell::EnergyFactorCell(const nlohmann::json &json) :
        ProductionCell(json), _energy_factor(json.at("energy_factor").get<long>()) {}
