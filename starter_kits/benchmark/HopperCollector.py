...
while True:
    ...
    command_queue = []
    dropoff_locations = [me.shipyard.position] + [dropoff.position for dropoff in me.get_dropoffs()]
    ...
    for ship in me.get_ships():  # For each of our ships
        ...
        if ship.position in dropoff_locations:
            state[ship.id] = None
        ...
        if state.get(ship.id) == 'return' or ship.halite_amount > 700:
            ...
            distance = 9999
            for dest in dropoff_locations:
                d = game_map.calculate_distance(ship.position, dest)
                if d < distance:
                    distance = d
                    destination = dest
