...
// Some defines for making certain things easier to understand
#define UNOCCUPIED ((unsigned short)-1)
...
{
	return map[x][y].occupied != UNOCCUPIED;
}

// Now for the commands to send to the game engine
inline void stay_still(unsigned short ship_id)
{
...
void naive_navigate(unsigned short ship_id)
{
	...
	unsigned char n_occupied = tile_occupied(x, norm_y(y - 1));
	unsigned char e_occupied = tile_occupied(norm_x(x + 1), y);
	unsigned char s_occupied = tile_occupied(x, norm_y(y + 1));
	unsigned char w_occupied = tile_occupied(norm_x(x - 1), y);

	// 10% chance each to move north/east/south/west (This is even smarter than before!)
	if (decision <= 10 && n_occupied == 0) {
		move_north(ship_id);
	} else if (decision <= 20 && e_occupied == 0) {
		move_east(ship_id);
	} else if (decision <= 30 && s_occupied == 0) {
		move_south(ship_id);
	} else if (decision <= 40 && w_occupied == 0) {
		move_west(ship_id);
	} else {
...
int main(int argc, char* argv[])
{
	...
	// The first input we get from the engine is some JSON-encoded constants. I'm just going to scanf this because the formatting is consistent
	...
	while (true) {
		...
		// And write another log stating it's the end of the turn! (don't forget to fflush!)
